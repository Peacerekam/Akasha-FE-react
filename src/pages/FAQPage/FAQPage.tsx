...
export const FAQPage: React.FC = () => {
  ...
  const highlightText = useCallback(
    (
      { textToHighlight, searchWords, autoEscape, ...rest }: FindChunks,
      id: number,
      DEBUG?: string,
    ) => {
      const count =
        textToHighlight.toLowerCase().split(("" + searchWords[0]).toLowerCase())
          .length - 1;
      ...
      setTimeout(() => {
        if (
          DEBUG &&
          !!("" + searchWords[0]).trim() &&
          !!textToHighlight.trim() &&
          wordHits[id]?.[`${id} - ${textToHighlight}`] !== count
        ) {
          console.log(
            'DEBUG', DEBUG,
            id,
            "\nwordHits[id]?.[textToHighlight]",
            wordHits[id]?.[`${id} - ${textToHighlight}`],
            count,
            !!textToHighlight.trim(),
            !!("" + searchWords[0]).trim(),
            searchWords[0]
          ...
          // setWordHits((prev: any) => ({
          //   ...prev,
          //   [id]: {
          //     ...prev[id],
          //     [`${id} - ${textToHighlight}`]: count,
          //   },
          // }));
        }
      }, 0);
      ...
      return chunks.filter((chunk) => chunk.highlight);
    },
    [wordHits]
  );
          ...
          <>
            <div className="faq-split">
              <div>
                ...
                <p>
                  <Highlighter
                    findChunks={(e) => highlightText(e, 0)}
                    highlightClassName="faq-header-highlight-class"
                    searchWords={[searchText]}
                    autoEscape={true}
          ...
          <>
            <div className="faq-split">
              <div>
                <p>
                  <Highlighter
                    findChunks={(e) => highlightText(e, 1)}
                    highlightClassName="faq-header-highlight-class"
                    searchWords={[searchText]}
                    autoEscape={true}
      ...
      <div className="content-block w-100" id="content-container">
        ...
        <div className="relative faq-page-content-wrapper">
          ...
          <div className="faq-page">
            ...
            <FAQBrowser contents={faqContents} wordHits={wordHits} />
          </div>
        </div>
      </div>
    </div>
